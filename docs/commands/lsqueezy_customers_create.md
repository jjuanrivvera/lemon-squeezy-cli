...
```
lsqueezy customers create [flags]
```

### Examples

```
  lsqueezy customers create --data '{"name":"Acme"}' --rel store=stores:1
  lsqueezy customers create --set name=Acme --set email=a@b.co --rel store=stores:1 --dry-run
```
