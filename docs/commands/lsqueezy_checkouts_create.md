...
```
lsqueezy checkouts create [flags]
```

### Examples

```
  lsqueezy checkouts create --data '{"name":"Acme"}' --rel store=stores:1
  lsqueezy checkouts create --set name=Acme --set email=a@b.co --rel store=stores:1 --dry-run
```
