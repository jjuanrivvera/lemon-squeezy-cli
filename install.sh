#!/bin/sh
# lsqueezy installer for macOS and Linux.
#
...
#   LSQUEEZY_VERSION=v1.2.3   pin a version (default: the latest release)
...
REPO="jjuanrivvera/lemon-squeezy-cli"
BINARY="lsqueezy"
VERSION="${LSQUEEZY_VERSION:-}"
...
if [ -z "$VERSION" ]; then
  ...
  [ -n "$VERSION" ] || die "could not determine the latest release; set LSQUEEZY_VERSION"
fi
